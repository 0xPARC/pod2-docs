...
pub struct SignedPod {
    ...
    // `key_string_map` is a hashmap to store the relation between key hashes and key strings
    key_string_map: HashMap<crate::Hash, String>,
}

impl SignedPod {
    pub fn new(params: &Params, kvs: HashMap<String, Value>) -> Result<Self> {
        let (hashed_kvs, key_string_map): (
            HashMap<crate::Hash, backend::Value>,
            HashMap<crate::Hash, String>,
        ) = kvs.iter().fold(
            (HashMap::new(), HashMap::new()),
            |(mut hashed_kvs, mut key_to_hash), (k, v)| {
                let h = hash_str(k);
                hashed_kvs.insert(h, backend::Value::from(v));
                key_to_hash.insert(h, k.clone());
